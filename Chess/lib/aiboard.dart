...
class aiboard extends StatefulWidget {
  const aiboard({super.key});

  @override
  State<aiboard> createState() => _aiboardState();
}
...
class _aiboardState extends State<aiboard> {
  ...
  List<int>?
  enPassantTarget; // Coordinates of square where en passant capture is possible
  ...
  try {
    ...
    // Set up a listener for engine responses
    game.stdout.listen((output) {
      print('Stockfish: $output');
      // Check for readyok response
    ...
    // Wait briefly for the engine to process the uci command
    await Future.delayed(const Duration(milliseconds: 500));
    
    // Configure engine settings
    game.stdin = 'setoption name Skill Level value $aiDifficultyLevel';
    game.stdin = 'setoption name Threads value 2';
    
    // Check if engine is ready
    game.stdin = 'isready';
    
    // Wait for readyok response (handled in the listener above)
  } catch (e) {
  ...
  // Convert board state to FEN (Forsyth-Edwards Notation) for Stockfish
  String getBoardFEN() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        if (piece == null) {
          ...
          // Map chess piece to FEN notation
          String pieceChar = '';
          switch (piece.name) {
          ...
          fen.write(piece.isBlack ? pieceChar : pieceChar.toUpperCase());
        }
      }
      ...
      // Add slash between ranks (except after the last rank)
      if (i < 7) {
        fen.write('/');
      }
    }
    ...
    if (enPassantTarget != null) {
      String file = String.fromCharCode(
        'a'.codeUnitAt(0) + enPassantTarget![1],
      );
      String rank = (8 - enPassantTarget![0]).toString();
      fen.write(' $file$rank ');
    } else {
      fen.write(' - ');
    }
  ...
  List<int> algebraicToCoordinates(String position) {
  ...
  String coordinatesToAlgebraic(int row, int col) {
 ...
 Future<void> getAIMove() async {
  if (!isblackturn) return; // Only make AI moves when it's black's turn
  
  try {
    // Show thinking indicator or message
    setState(() {
      // You could add a loading indicator here
    });
    
    // Send current position to Stockfish
    String fen = getBoardFEN();
    game.stdin = 'position fen $fen';
    
    // Adjust thinking time based on difficulty
    int thinkTime = 300 + (aiDifficultyLevel * 100); // 400ms to 2300ms
    game.stdin = 'go movetime $thinkTime';
    
    // Wait for Stockfish to return the best move with a timeout
    String? bestMove;
    bool moveReceived = false;
    
    // Create a subscription to listen for the bestmove response
    var subscription = game.stdout.listen((output) {
    ...
    await Future.delayed(Duration(milliseconds: thinkTime + 2000));
    
    // Clean up the subscription
    await subscription.cancel();
    
    if (!moveReceived) {
      print('Stockfish did not return a move in time');
      makeSimpleAIMove(); // Fall back to simple AI
      return;
    }
    
    if (bestMove != null && bestMove!.length >= 4) {
      // Extract start and end positions
      String fromAlgebraic = bestMove!.substring(0, 2);
      String toAlgebraic = bestMove!.substring(2, 4);
      
      List<int> fromCoords = algebraicToCoordinates(fromAlgebraic);
      List<int> toCoords = algebraicToCoordinates(toAlgebraic);
      ...
      if (piece != null) {
        ...
        selectedRow = fromRow;
        selectedCol = fromCol;
        validmoves = calculaterealmoves(fromRow, fromCol, piece);
        
        // Make the move
        movepiece(toCoords[0], toCoords[1]);
      }
    }
  } catch (e) {
  ...
  void pieceSelected(int row, int col) {
    ...
    setState(() {
      ...
      if (selectedPiece == null) {
        ...
        // Only allow selection of white pieces (human player)
  ...
  bool isInCheckmate(bool isBlackKing) {
    ...
    List<List<int>> kingMoves = calculaterealmoves(
      kingPosition[0],
      kingPosition[1],
      king,
    );
  ...
  Widget _buildPromotionOption(
    ...
    String imagePath =
        isBlack
            ? "pieces/W_${label.toLowerCase()}.png"
            : "pieces/W_${label.toLowerCase()}.png";
  ...
  void movepiece(int row, int col) {
    ...
    if (movingPiece.name == ChessPieces.whitePawn) {
      if ((movingPiece.isBlack && row == 7) ||
          (!movingPiece.isBlack && row == 0)) {
    ...
    if ((movingPiece.name == ChessPieces.whiteKing ||
            movingPiece.name == ChessPieces.blackking) &&
        (col == selectedCol + 2 || col == selectedCol - 2)) {
      isCastling = true;
      castlingDirection =
          col > selectedCol ? 1 : -1; // 1 for kingside, -1 for queenside
    }
    ...
    } else if (movingPiece.name == ChessPieces.whiteRook &&
        movingPiece.isBlack) {
    ...
    if (movingPiece.name == ChessPieces.whitePawn &&
      ...
      int enPassantRow =
          movingPiece.isBlack ? selectedRow + 1 : selectedRow - 1;
  ...
  void finalizeMoveAndSwitchTurn() {
    ...
    // If it's now black's turn, trigger the AI move
    if (isblackturn) {
      // Add a small delay to make the move seem more natural
  ...
  List<List<int>> calculaterawmoves(int row, int col, ChessPiece? piece) {
    ...
    switch (piece.name) {
      case ChessPieces.whitePawn:
        ...
        // En passant captures
        if (enPassantTarget != null &&
            inboard(enPassantTarget![0], enPassantTarget![1])) {
          if (row + direction == enPassantTarget![0] &&
      ...
      case ChessPieces.whiteRook:
        ...
        for (int d = 1; d < 8; d++) {
          ...
          } else if (inboard(row + d, col) &&
              board[row + d][col]?.isBlack != piece.isBlack) {
            candidatemoves.add([row + d, col]);
            break;
          } else {
            break;
        ...
        for (int d = 1; d < 8; d++) {
          ...
          } else if (inboard(row, col + d) &&
              board[row][col + d]?.isBlack != piece.isBlack) {
            candidatemoves.add([row, col + d]);
            break;
          } else {
            break;
      ...
      case ChessPieces.whiteBishop:
        ...
        for (int d = 1; d < 8; d++) {
          ...
          } else if (inboard(row + d, col + d) &&
              board[row + d][col + d]?.isBlack != piece.isBlack) {
            candidatemoves.add([row + d, col + d]);
            break;
          } else {
            break;
  ...
  bool simulation(
    ...
    if ((piece.name == ChessPieces.whiteKing ||
            piece.name == ChessPieces.blackking) &&
  ...
  @override
  void dispose() {
    // Properly close the Stockfish process
    try {
      game.stdin = 'quit';
      // Small delay to allow the process to terminate gracefully
  ...
  // Add this method to make a simple AI move when Stockfish isn't available
  void makeSimpleAIMove() {
    if (!isblackturn) return;

    // Find all black pieces that can move
    List<List<int>> piecePositions = [];
    List<List<List<int>>> validMovesList = [];

    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        if (board[i][j] != null && board[i][j]!.isBlack) {
          List<List<int>> moves = calculaterealmoves(i, j, board[i][j]);
          if (moves.isNotEmpty) {
    ...
    if (piecePositions.isEmpty) return;

    // Choose a random piece and move
    Random random = Random();
    int pieceIndex = random.nextInt(piecePositions.length);
    int moveIndex = random.nextInt(validMovesList[pieceIndex].length);

    int fromRow = piecePositions[pieceIndex][0];
    int fromCol = piecePositions[pieceIndex][1];
    int toRow = validMovesList[pieceIndex][moveIndex][0];
    int toCol = validMovesList[pieceIndex][moveIndex][1];

    // Select and move the piece
    selectedPiece = board[fromRow][fromCol];
    selectedRow = fromRow;
    selectedCol = fromCol;
    validmoves = validMovesList[pieceIndex];
  ...
  void showDifficultyDialog() {
    int tempDifficulty = aiDifficultyLevel;
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: StatefulBuilder(
            builder: (BuildContext context, StateSetter setState) {
              return Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text('Difficulty Level: $tempDifficulty'),
                  Slider(
                    value: tempDifficulty.toDouble(),
                    min: 1,
                    max: 20,
                    divisions: 19,
                    label: tempDifficulty.toString(),
                    onChanged: (double value) {
                      setState(() {
                        tempDifficulty = value.round();
                      });
                    },
                  ),
                  const Text(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                setAIDifficulty(tempDifficulty);
                Navigator.of(context).pop();
              },
  ...
  void setAIDifficulty(int level) {
    ...
    try {
      // Send the new difficulty level to Stockfish
      game.stdin = 'setoption name Skill Level value $level';
      // Also adjust search depth based on difficulty
      game.stdin = 'setoption name Contempt value ${(level - 10) * 10}';
      game.stdin = 'setoption name MultiPV value ${20 - level}';

      // For very low difficulty levels, limit thinking time
      if (level <= 5) {
        game.stdin = 'setoption name Move Overhead value 500';
      } else {
        game.stdin = 'setoption name Move Overhead value 30';
      }

      print('AI difficulty set to level $level');
    } catch (e) {
      print('Error setting AI difficulty: $e');
    }
  }
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              if (enPassantTarget != null) const SizedBox(width: 10),
              const Text(
                "",
                style: TextStyle(
                  color: Colors.yellow,
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Square(
                  ...
                  isSelected:
                      selectedRow == row &&
                      selectedCol == col, // Check if the square is selected
                  onTap: () {
          ...
          Row(
            ...
            children: [
              Text(
                isblackturn ? "Black's Turn (AI)" : "White's Turn (You)",
